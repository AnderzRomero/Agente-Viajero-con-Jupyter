...
`miles_graph()` returns an undirected graph over 128 US ciudades. The
ciudades each have location and population data.  The edges are labeled with the
distance between the two ciudades.
...
def miles_graph():
    ...
    for line in fh.readlines():
        ...
        if numfind.match(line):  # this line is distances
            dist = line.split()
            for d in dist:
                G.add_edge(city, ciudades[i], weight=int(d))
                i = i + 1
        else:  # this line is a city, position, population
            i = 1
            (city, coordpop) = line.split("[")
            ciudades.insert(0, city)
...
print("Loaded miles_dat.txt containing 128 ciudades.")
print(G)
...
try:
    ...
    # Plot edges between the ciudades
    for edge in H.edges():
